...
2. ~~Карту переписати на `d3-geo` React-компонент.~~ Зроблено: геометрія
   проєктується на етапі збірки (`scripts/europe-map.mjs` → `src/content/europe-map.json`),
   компонент `EventsMap` малює звичайний SVG. Жодних зовнішніх запитів.
3. Підняти Payload CMS (Node LTS) + БД, реалізувати `PayloadRepository`.
4. ~~Мігрувати `/reader`, `/atlas` під локалі.~~ Ці маршрути видалено —
   вони лишалися під старим брендом caseflows і індексувалися.
