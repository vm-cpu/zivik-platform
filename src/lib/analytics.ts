 ...
 *
 * Прапорець збірки, а не запиту — як SITE_INDEXABLE (`lib/seo.ts`): усі
 * сторінки пререндерені, тож «є маячок чи ні» — це властивість збірки. Одне
 * значення вмикає чотири речі разом, і жодна не може розійтися з іншою:
